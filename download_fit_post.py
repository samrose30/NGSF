...
if args.wav_range == None:
    wav_min, wav_max = None, None

elif args.wav_range != None:
...
if z == None:
    ...
    cmd = str('python run.py ' + str(to_fit_dir) + str(spec_filename) + ' ' + '100' + ' ' + wav_min + ' ' + wav_max)
    os.system(cmd)
    ...
    fit_z = df['Z'][0]
    cmd_z = str('python run.py ' + str(to_fit_dir) + str(spec_filename) + ' ' + str(fit_z) + ' ' + wav_min + ' ' + wav_max)
    os.system(cmd_z)
    z = fit_z
    images = []
    ...
    text = str('Top 3 matches from superfit with redshift a free parameter (row 1) and with redshift fixed (row 2) for ' + 
    prefix + ' fritz specid ' + specid + ' ran at ' + now_str)

elif float(z) != 100:
    cmd = str('python run.py ' + str(to_fit_dir) + str(spec_filename) + ' ' + str(z) + ' ' + wav_min + ' ' + wav_max)
    os.system(cmd)
    images = []
    prefix = spec_filename.split('.')[0]
    ...
    now_str = str(datetime.now())
    text = str('Top 3 matches from superfit with redshift fixed to the Fritz value z = ' + str(z) + ' for ' + 
            prefix + ' fritz specid ' + specid + ' ran at ' + now_str)
